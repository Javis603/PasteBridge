...
6. 在兩台機器設定相同的 `AUTH_TOKEN`。

7. 在 Windows 的 `.env` 把 `SERVER_IP` 設成 Mac 可連到的 IP，建議使用 ZeroTier、Tailscale 或同一個 LAN。
